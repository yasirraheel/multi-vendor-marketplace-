...
<div class="box">
  <div class="box-header with-border">
    <h3 class="box-title">
      {{ $category->hasTranslation($selected_language) ? trans('app.update_model_translation', ['model' => trans('app.model.category')]) : trans('app.add_model_translation', ['model' => trans('app.model.category')]) }}
      | {{ trans('app.name') . ':' }} {{ $category->name }}
    </h3>
  </div>
  <div class="box-body">
    <div class="row">
    ...
    <div class="box-tools pull-right">
      <button type="submit" class="btn btn-flat btn-lg btn-primary">{{ $category->hasTranslation($selected_language) ? trans('app.form.update') : trans('app.form.save') }}</button>
    </div>
  </div>
</div>
